...
func handleAcParam(w http.ResponseWriter, r *http.Request) {
	...
	cred, credPlain, ekPub, aikPub, err := verifyActivateCredParameters(body)
	if err != nil {
		fmt.Println(handleHTTPError(w, http.StatusBadRequest, "failed to verify activate credential parameters: %v", err))
		return
	}

	// this is very basic device registration and has no real-word checks and balances
	deviceID, err := registerDevice(ekPub, aikPub, credPlain)
	if err != nil {
...
func handleAcActive(w http.ResponseWriter, r *http.Request) {
	...
	if verfiyCredentialActivation(body, device.cred) != nil {
	...
	fmt.Println("\tDevice AIK is now trusted")
	w.WriteHeader(http.StatusOK)
}
